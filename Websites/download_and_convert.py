...
def download_and_convert(src, download_path, chp_nb, referer=''):
    
    img_list = []
    for j in range(len(src)):
        ext = src[j].split('.')[-1]

        download_image(src[j], download_path + 'temp' + os.path.sep, 'chp_' + chp_nb[1] + '_' + str(j) + '.' + ext, referer=referer + chp_nb[0])
